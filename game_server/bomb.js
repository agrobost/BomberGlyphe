...
var bomb = function (_personnage, cell) {
    ...
    var timerExplosion;
    env.map[cell.x][cell.y] = new Bomb();


    //on met a jour la map car une bombe vient d'etre posé
    personnage.io.sockets.in(personnage.refGame).emit("modify cell", {
        i: cell.x,
        j: cell.y,
    ...
    this.explosion = function () {
        ...
        function test() {
            ...
            if (stop1 !== true) {
                stop1 = env.cellBelongToMap({
                    x: cell.x - j,
                    y: cell.y
                }) ? toDo(cellAffected, cell.x - j, cell.y, env.map) : true;
            }
            if (stop2 !== true) {
                stop2 = env.cellBelongToMap({
                    x: cell.x,
                    y: cell.y - j
                }) ? toDo(cellAffected, cell.x, cell.y - j, env.map) : true;
            }
            if (stop3 !== true) {
                stop3 = env.cellBelongToMap({
                    x: cell.x + j,
                    y: cell.y
                }) ? toDo(cellAffected, cell.x + j, cell.y, env.map) : true;
            }
            if (stop4 !== true) {
                stop4 = env.cellBelongToMap({
                    x: cell.x,
                    y: cell.y + j
                }) ? toDo(cellAffected, cell.x, cell.y + j, env.map) : true;
            }
        ...
        function toDo(cellAffected, x, y) {
            var etendre = false;
